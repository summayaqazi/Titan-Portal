...
export default function Drawer({ open, onClose, title, children, footer, width = 'w-[420px]' }) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex cursor-pointer justify-end bg-slate-900/40"
      onMouseDown={(e) => {
    ...
    >
      <div
        ...
        // element inside the actual drawer (not just its buttons/links,
        // which already get their own pointer via the global rule) would
        // incorrectly show a pointer cursor too.
        className={`flex h-full ${width} max-w-full cursor-auto flex-col bg-white shadow-xl`}
      >
